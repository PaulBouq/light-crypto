...
\part{Présentation de deux algorithmes de cryptographie légère}
	...
	et il a la particularité d’avoir été inclus au dernier standard international des méthodes de cryptographie légère par l’ « International Electrotechnical Commission » [3].
		...
		La figure 1 schématise le fonctionnement de l’algorithme, qui s’organise en trois phases distinctes, que sont l’initialisation, l’absorption (absorbing) et le relâchement (squeezing).
		...
		Le message est ensuite découpé en blocs de r bits. Ensuite vient la phase d’absorption.
		...
		Les valeurs de n, b, c et r ne sont pas prises au hasard. En effet, il existe treize versions différentes de SPONGENT et elles sont définies grâce au tableau représenté à la figure 2.
		...
		Cette sous-partie va permettre d’expliquer le fonctionnement détaillé de la permutation notée b??. Cette permutation est définie par l’algorithme représenté à la figure 3 :
		% image 3
		...
		La figure 5 illustre les polynômes générateurs du LFSR, et la figure 4 indique les valeurs initiales que prend le LFSR.
		% image 4 | image 5
		La deuxième opération de la ronde est de passer STATE dans une S-Box.
		La S-Box est la même pour toutes les variantes de l’algorithme SPONGENT et elle est définie par la figure 6 :
		% image 6
		Enfin, la troisième et dernière opération est de passer STATE dans pLayerb.
		pLayerb est une fonction de permutation définie comme l’inverse de la permutation de bits de l’algorithme PRESENT. Ainsi, chaque bit j est permuté avec le bit de position Pb(j) où :
		% formule
